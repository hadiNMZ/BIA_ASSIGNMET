...
class AlgorithmData:
    ...
    def all_product_ids(self) -> list[int]:
        return self.products["product_id"].tolist()
...
class AlgorithmDataImporter:
    ...
    async def get_ratings(self) -> pd.DataFrame:
        async with self.session_factory() as session:
            ...
            return pd.DataFrame(
                ...
                columns=["id", "user_id", "product_id", "rating"],
            )

    async def get_behaviors(self) -> pd.DataFrame:
        async with self.session_factory() as session:
